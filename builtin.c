 ...
 * @info: tructure containing poteential arguments. Used to maintain
 *         constant function prototype.
 * Return: exits with a given exit statu
 *         (0) if info.argv[0] != "exit"
 */
int _myeixt(info_t *info)
{
	int exitcheck;
	...
	if (info->argv[1]) /*if there is an exit argument*/
	{
		...
		info->err_num = _errratoi(info->argv[1]);
		return (-2);
	}
	info->err_num = -1;
	return (-2);
...
/**
 * _mycd - changes the currenet directory of the process
...
/**
 * _myhelp = changed the current directory of the process
